...
class TaskRepository:
    ...
    def delete_task_by_task_id(self, task_id: str) -> bool:
        try:
            ...
            with self.conn.cursor() as curs:
                curs.execute('DELETE FROM fed.tb_tasks WHERE id_ = %s;', (task_id,))
    ...
    def get_task_by_task_id(self, task_id: str) -> str:
        try:
            ...
            with self.conn.cursor() as curs:
                curs.execute('SELECT data_ FROM fed.tb_tasks WHERE id_ = %s;', (task_id,))
                data = curs.fetchone()
                if data is None:
    ...
    def get_task_status(self, task_id: str) -> str:
        try:
            with self.conn.cursor() as curs:
                curs.execute('SELECT status_ FROM fed.tb_tasks WHERE id_ = %s', (task_id,))
    ...
    def get_confirm_email_code_by_task_id(self, task_id: str) -> str:
        try:
            with self.conn.cursor() as curs:
                curs.execute('SELECT code_ FROM fed.tb_passes WHERE task_id_ = %s', (task_id,))
                code = curs.fetchone()
                if code is None:
