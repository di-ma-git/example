...
class TestE2EContractFvno:
    ...
    _token: str
    _headers_front_with_auth: str
    _view: str
    _confirm_email_code: str
    _nationality: str
    _installer_code: str
    ...
    def test_e2e_contract_fvno(self, task_repository, source_value, type_task, provider_value):
        ...
        payload_register_post = {"phone": self._identificationValue}
        result_register_post = requests.request(
            "POST",
            "http://devapp.homeinternet.rt.ru/api/v1/register",
            headers=Data.headers_front,
            json=payload_register_post,
            verify=False
        )
        assert result_register_post.status_code == HTTPStatus.OK
        time.sleep(1)

        result_login_get = requests.get(
            f"https://devapp.homeinternet.rt.ru/api/v1/login?phone={self._identificationValue}",
            headers=Data.headers_front,
            verify=False
        )
        assert result_login_get.status_code == HTTPStatus.OK
        time.sleep(1)

        payload_login_post = {"password": "111111", "phone": self._identificationValue}
        result_login_post = requests.request(
            "POST",
            "http://devapp.homeinternet.rt.ru/api/v1/login",
            headers=Data.headers_front,
            json=payload_login_post,
            verify=False
        )
        assert result_login_post.status_code == HTTPStatus.OK
        self._token = result_login_post.json().get("contents", {}).get("token")
        assert self._token
        time.sleep(1)

        self._headers_front_with_auth = Data.headers_front.copy()
        self._headers_front_with_auth["Authorization"] = f"Bearer {self._token}"

        result_task_get = requests.get(
            "http://devapp.homeinternet.rt.ru/api/v1/task",
            headers=self._headers_front_with_auth,
            verify=False
        )
        assert result_task_get.status_code == HTTPStatus.OK
        assert result_task_get.json()["contents"]["tasks"][0]["taskId"] == create_response.json().get("taskId")
        time.sleep(2)

        payload_task_post = {"taskId": self._task_id}
        result_task_post = requests.post(
            "http://devapp.homeinternet.rt.ru/api/v1/task",
            headers=self._headers_front_with_auth,
            json=payload_task_post,
            verify=False
        )
        assert result_task_post.status_code == HTTPStatus.OK
        assert result_task_post.json().get("status") == "success"
        time.sleep(1)
        ...
        assert result_step_get.json().get("contents", {}).get("view") == "FVNOOffer"
        self._view = result_step_get.json().get("contents", {}).get("view")
        time.sleep(1)

        result_settings_get = requests.get(
            "http://devapp.homeinternet.rt.ru/api/v1/contract/settings",
            headers=self._headers_front_with_auth,
            verify=False
        )
        assert result_settings_get.status_code == HTTPStatus.OK
        assert result_settings_get.json().get("contents", {}).get("success") == True
        time.sleep(2)

        payload_next_post = {"from": self._view}
        ...
        assert result_step_get.status_code == HTTPStatus.OK
        assert result_step_get.json().get("contents", {}).get("view") == "FVNOClient"
        self._view = result_step_get.json().get("contents", {}).get("view")
        time.sleep(1)

        result_contract_get = requests.get(
            "http://devapp.homeinternet.rt.ru/api/v1/contract?" +
            "fields%5B%5D=codeWord&" +
            "fields%5B%5D=consentToSmsInform&" +
            "fields%5B%5D=email&" +
            "fields%5B%5D=inn&" +
        ...
        payload_next_post = {
            "data": {
                "codeWord": f"{result_contract_get.json().get("contents", {}).get("codeWord", {}).get("value")}",
                "consentToSmsInform": f"{result_contract_get.json().get("contents", {}).get("consentToSmsInform", {}).get("value")}",
                "email": f"{result_contract_get.json().get("contents", {}).get("email", {}).get("value")}",
                "inn": f"{result_contract_get.json().get("contents", {}).get("inn", {}).get("value")}",
                "middleName": f"{result_contract_get.json().get("contents", {}).get("middleName", {}).get("value")}",
                "name": f"{result_contract_get.json().get("contents", {}).get("name", {}).get("value")}",
                "nationality": f"{result_contract_get.json().get("contents", {}).get("nationality", {}).get("value")}",
                "phone": f"{result_contract_get.json().get("contents", {}).get("phone", {}).get("value")}",
                "sex": f"{result_contract_get.json().get("contents", {}).get("sex", {}).get("value")}",
                "snils": f"{result_contract_get.json().get("contents", {}).get("snils", {}).get("value")}",
                "surname": f"{result_contract_get.json().get("contents", {}).get("surname", {}).get("value")}"
            },
            "from": f"{self._view}"
        }
        result_next_post = requests.post(
        ...
        assert result_step_get.status_code == HTTPStatus.OK
        assert result_step_get.json().get("contents", {}).get("view") == "FVNOEmailConfirm"
        self._view = result_step_get.json().get("contents", {}).get("view")
        time.sleep(1)
